...
# Define key variables (check if they exist in dataset)
malaria_outcome_vars <- c("study_arm", 
...
malaria_outcome_vars <- intersect(malaria_outcome_vars, colnames(PROMO_Data))

#  Generate the table only if dataset has rows
if (nrow(PROMO_Data) > 0) {
  malaria_outcomes_table <- PROMO_Data %>%
    select(all_of(malaria_outcome_vars)) %>%
    tbl_summary(
      by = study_arm,  # Compare DP vs. SP groups
      statistic = list(
        total_malaria_episodes_during_pregnancy ~ "{mean} ± {sd}",  # Show mean ± SD
    ...
    modify_header(label = "**Variable**") %>%  # Ensure readable column headers
    bold_labels()  # Bold table labels for clarity
